...
import com.flyang.view.layout.refresh.constant.DimensionStatus;
...
import com.flyang.view.layout.refresh.wrapper.RefreshFooterWrapper;
import com.flyang.view.layout.refresh.wrapper.RefreshHeaderWrapper;
...
public class SmartRefreshLayout extends ViewGroup implements RefreshLayout, NestedScrollingParent/*, NestedScrollingChild*/ {
    ...
    protected void onFinishInflate() {
        ...
        if (mEnablePureScrollMode) {
            PreconditionUtils.checkPositionIndex(count, 1, "存滚动模式,只支持1个子View");
        } else {
            PreconditionUtils.checkPositionIndex(count, 3, "最多只支持3个子View");
        }
    ...
    protected void moveSpinnerInfinitely(float spinner) {
        ...
        if (mNestedInProgress && !mEnableLoadMoreWhenContentNotFull && spinner < 0) {
            if (!mRefreshContent.canLoadMore()) {
                // 修复 嵌套滚动模式下 mEnableLoadMoreWhenContentNotFull=false 无效的bug
                spinner = 0;
            }
        }
