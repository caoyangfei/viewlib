package com.flyang.view.layout.swipeback;
...
public class BGASwipeBackShadowView extends FrameLayout {
    ...
    public void bindPreActivity() {
        ...
        if (mPreActivity == null) {
            Activity preActivity = ApplicationUtils.getActivityLifecycle().getPenultimateActivity(mActivity);
            if (preActivity != null) {
    ...
    public void unBindPreActivity(boolean isNeedAddImageView) {
        if (mIsCurrentActivityTranslucent) {
            return;
        }
    ...
    private boolean containsProblemView(ViewGroup viewGroup) {
        ...
        for (int i = 0; i < childCount; i++) {
            ...
            if (ApplicationUtils.getActivityLifecycle().isProblemView(childView)) {
                return true;
    ...
    private void onPanelSlide(Activity currentActivity, float slideOffset) {
        try {
            Activity preActivity = ApplicationUtils.getActivityLifecycle().getPenultimateActivity(currentActivity);
            if (preActivity != null) {
